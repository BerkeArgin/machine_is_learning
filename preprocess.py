...
def select_categorical(x_train):
    ...
    categorical.
    """
    unique_values_count = np.array(
        [len(np.unique(column[~np.isnan(column)])) for column in x_train.T]
    )
...
def remove_highly_correlated_features(X_train, X_test, threshold=0.95):
    ...
    reduced_X_test = (
        np.delete(X_test, list(columns_to_remove), axis=1)
...
def apply_preprocessing(x_train, x_test, coefficient=2):
    ...
    x_train, mean, std_x = handle_outliers(
        x_train, numerical_columns_indices, coefficient
    )
    x_test, _, _ = handle_outliers(
        x_test, numerical_columns_indices, coefficient, mean, std_x
    )
    # print("imhere")
